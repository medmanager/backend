...
import { getScheduledDays } from "./controller";
...
export const scheduleNewMedication = async (user, medication) => {
    ...
    let scheduledDays = getScheduledDays(user);

    scheduledDays.forEach((day) => {
        day.forEach((dosageOccurrence) => {
            // I believe the loop and conditional below is unnecessary, but I could be wrong.
            // user.medications.forEach((uMed) => {
            //     if (
            //         med.medicationId == medication._id &&
            //         medication._id == uMed._id
            //     ) {
            //         console.log(`found new med to schedule ${uMed._id}`);
            //found new med to schedule
            dosageOccurrence.datesWTime.forEach((dose) => {
                medication.dosages.forEach((uDose) => {
                    let now = new Date();
                    if (
                        dose.dosageId.equals(uDose._id) &&
                        now.getTime() < dose.date.getTime()
                    ) {
                        //create occurrence
                        let occurrence = {
                            isTaken: false,
